...
public class BrokerCustomerService {
	...
	public void saveOrUpdate(BrokerCustomer userObj) {
		...
		BrokerCustomer brokerContomer=BrokerCustomer.builder().userName(userObj.getUserName()).profileFor(userObj.getProfileFor())
				.phoneNo(userObj.getPhoneNo()).gender(userObj.getGender()).dateOfBirth(userObj.getDateOfBirth())
				.religion(userObj.getReligion()).motherTounge(userObj.getMotherTounge()).email(userObj.getEmail())
				.password(userObj.getPassword()).caste(userObj.getCaste()).stateId(userObj.getStateId()).cityId(userObj.getCityId())
				...
				.highestEducation(userObj.getHighestEducation()).annualIncome(userObj.getAnnualIncome()).occupation(userObj.getOccupation())
				.about(userObj.getAbout()).jobType(userObj.getJobType()).name(userObj.getName()).otp(otps)
				.isOtpVerified(userObj.getIsOtpVerified()).expiryDate(expireDate).build();
