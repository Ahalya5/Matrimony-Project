...
public interface BrokerCustomerRepository extends WriteableRepository<BrokerCustomer, UUID> {
	...
	@Query(value = "Select * from mat_broker_customer bc where bc.phone_no =:phoneNo ",nativeQuery = true)
	public Optional<BrokerCustomer> findByPhoneNo(String phoneNo);
	
	Optional<BrokerCustomer> findByUserName(String name);
	
	@Query(value = "select * from mat_broker_customer bc where bc.email=:email",nativeQuery = true)
	Optional<BrokerCustomer> findByEmail(String email);
	
	@Query(value = "select * from mat_broker_customer bc where bc.role_type =:admin",nativeQuery = true)
